...
from myWordMap_misc import generate2_adpt_if, evaluate_on_coco_caption
...
class ClipCocoDataset(Dataset):
    ...
    def pad_tokens(self, item: int):
        encoded_output = self.tokenizer.encode('endoftext')
        tokens = torch.cat((self.captions_tokens[item], torch.tensor(encoded_output.ids)), dim=0)
        gt = torch.cat((self.captions_tokens[item], torch.tensor(encoded_output.ids)), dim=0)
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        # train+restval
        # print(int(img_id)//5)
        filename = f"{self.data_root}{'/'}{img_id}"
        image = io1.imread(filename)
        image = Image.fromarray(image)
        image = self.preprocess(image)

        # TODO this is for pre-computed clip feature
    ...
    def __init__(self, data_root: str, data_path: str, gpt2_type: str = "gpt2", len_seq=20, normalize_prefix=False, cut_num=None):
        self.data_root = data_root
        # self.tokenizer = GPT2Tokenizer.from_pretrained(gpt2_type)
        myWordMap_dir = os.path.join(os.path.dirname(data_root), 'wordLevel_Tokenizer.json')
        self.tokenizer = Tokenizer.from_file(myWordMap_dir)
        ...
        if cut_num:
            num_to_extract = int(len(self.prefixes) * cut_num / 100)
            self.prefixes = self.prefixes[:num_to_extract]
            captions_raw = captions_raw[:num_to_extract]
        print("captions_tokens %0d" % len(self.prefixes))
        print("captions_raw is %0d" % len(captions_raw))
        ...
        if os.path.isfile(f"{data_path[:-4]}_tokens.pkl"):
            ...
            for caption in captions_raw:
                self.captions_tokens.append(torch.tensor(self.tokenizer.encode(caption['caption']).ids, dtype=torch.int64))
                # print(caption["clip_embedding"])
...
class ClipCocoValDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        filename = f"{self.data_root}{'/'}{_filename['image_id']}"
        # print(filename)
        for x in self.test_annotation:
            if str(x['image_id']) == _filename['image_id']:
                caption = x['caption']
                break
        tokens = torch.tensor(self.tokenizer.encode(caption).ids, dtype=torch.int64)
        tokens = torch.cat((tokens, torch.tensor(self.tokenizer.encode('endoftext').ids)), dim=0)
    ...
    def __init__(self, data_root: str, cut_num=None):
        self.data_root = data_root
        myWordMap_dir = os.path.join(os.path.dirname(data_root), 'wordLevel_Tokenizer.json')
        self.tokenizer = Tokenizer.from_file(myWordMap_dir)
        val_path = os.path.join(os.path.dirname(self.data_root), 'captions_val.json')
        test_path = os.path.join(os.path.dirname(self.data_root), 'captions_test.json')
        self.annotation = json.load(open(val_path, "r"))["annotations"]
        self.test_annotation = json.load(open(test_path, "r"))["annotations"]
        with open(test_path, 'r') as j:
            self.files = json.load(j)['annotations']
...
class ClipCaptionModel(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, mask_tokens: torch.Tensor, prefix: torch.Tensor,
        ...
        embedding_text = torch.where(mask_tokens == self.word_len, bos_token_embedding, embedding_text)
    ...
    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 512,
                 num_layers: int = 8, mapping_type: MappingType = MappingType.MLP, Timestep: int = 20,
                 if_drop_rate=0.02, param=None):
        super(ClipCaptionModel, self).__init__()
        self.prefix_length = prefix_length
        self.args = param
        myWordMap_dir = os.path.join(os.path.dirname(self.args.data_root), 'wordLevel_Tokenizer.json')
        with open(myWordMap_dir, 'r') as j:
            data = json.load(j)
        self.word_len = len(data['model']['vocab'])
        ...
        configuration.update({'vocab_size': self.word_len})
        configuration.update({'bos_token_id': int(self.word_len) - 1})
        ...
        self.num_classes = self.word_len + 1
        ...
        self.pad_embedding = nn.Parameter(
            torch.randn(size=(196, 768), requires_grad=True, dtype=torch.float64))  # image free vector
...
def val(model, epoch, val_dataloader, args):
    model.eval()
    myWordMap_dir = os.path.join(os.path.dirname(args.data_root), 'wordLevel_Tokenizer.json')
    tokenizer = Tokenizer.from_file(myWordMap_dir)
    ...
    # for index, value in enumerate(val_dataloader):
    #     a = value
    #     print(len(value))
    for idx, (image, image_path, tokens, gt, mask) in enumerate(val_dataloader):
        # print(idx, (image, image_path, tokens, gt, mask))
        ...
        for i_t in range(0, model.module.time_step):
            ...
            with open(myWordMap_dir, 'r') as j:
                data = json.load(j)
            word_len = len(data['model']['vocab'])
            all_mask = torch.where(mask_tokens == word_len, ex_mask, ex_nomask)
        ...
        if args.use_beam_search:
            ...
            generated_text_prefix = generate2_adpt_if(model, tokenizer, embed=prefix_embed,
                                                      len_pre=len_pre.argmax(-1) + 1)

        torch.cuda.synchronize()
        progress.update()
        r = [{'image_id': _image_path, 'id': _image_id, 'result': _text} for _image_path, _image_id, _text in
             zip(image_path['image_id'], image_path['id'].numpy(), generated_text_prefix)]
        result_all.extend(r)
    progress.close()
    os.makedirs(f'./cache/{args.tag}', exist_ok=True)
    temp = {'annotations': result_all}
    json.dump(temp, open(f"./cache/{args.tag}/tmp-results-{dist.get_rank()}.json", "w"), cls=NpEncoder)
    ...
    if dist.get_rank() == 0:
        ...
        for i in range(dist.get_world_size()):
            ...
            for ep in part_result['annotations']:
                if ep['id'] not in ra_id:
                    ra_id.append(ep['id'])
                    result_all.append(ep)
        result_all = {"annotations": result_all}
        test_annotation = os.path.join(os.path.dirname(args.data_root), 'captions_test.json')
        # save_result_score_root = os.path.join(os.path.dirname(args.data_root), 'result_score.json')
...
def train(model, epoch, train_dataloader, optimizer, lr_scheduler, scaler, args,
    ...
    for idx, (tokens, mask, prefix, gt) in enumerate(train_dataloader):
        ...
        progress.set_postfix(
            {"loss": loss.item() - loss_len.item(), 'lr': optimizer.param_groups[0]['lr'], 'loss_len': loss_len.item(),
             "loss_all": loss.item()})
        progress.update()
    progress.close()
    return model
...
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='../dataset/UCM_Caption/train_image_raw_captions.pkl')
    parser.add_argument('--data_root', default='../dataset/UCM_Caption/img')
    parser.add_argument('--out_dir', default='./results_UCM')
    parser.add_argument('--cut', default=80)
    ...
    parser.add_argument('--tag', default='clip_myword_bs128_lr2e4_cut',
...
def main(args):
    ...
    if args.only_prefix:
        ...
        model = ClipCaptionModel(args.prefix_length, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
                                 num_layers=args.num_layers, mapping_type=args.mapping_type, Timestep=args.time_step,
                                 if_drop_rate=args.if_drop_rate, param=args)
    ...
    dataset = ClipCocoDataset(args.data_root, args.data, normalize_prefix=args.normalize_prefix, cut_num=args.cut)
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset)
    train_dataloader = DataLoader(dataset, batch_size=args.bs, sampler=train_sampler, num_workers=8, pin_memory=True,
                                  drop_last=True)

    val_dataset = ClipCocoValDataset(args.data_root)
    val_sampler = torch.utils.data.distributed.DistributedSampler(val_dataset, shuffle=False)
    val_dataloader = DataLoader(val_dataset, batch_size=args.bs, sampler=val_sampler, num_workers=8, pin_memory=True,
                                drop_last=False)

    # result = val(model, 0, val_dataloader, args)
    ...
    for epoch in range(args.epochs):
        _ = train(model, epoch, train_dataloader, optimizer, lr_scheduler, scaler, args, output_dir=args.out_dir,
